<template>
  <div :key="personData?.useraname" class="flex flex-col items-start justify-start w-full divide-y divide-gray-500">
    <div class="flex flex-row items-start justify-start w-full my-6">
      <div class="flex flex-col items-start justify-start">
        <div>
          <span class="text-xl font-medium">Name</span>: {{personData?.displayName?.toLowerCase()?.replace(/\b\w/g, (match) => match.toUpperCase()) ?? ""}}
        </div>
        <div>
          <span class="text-xl font-medium">Course</span>: {{ getCourse(personData) }}
        </div>
      </div>
